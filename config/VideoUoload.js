...
const UploadToServer = async (req, res) => {
    try {
        ...
        const videoUrl = `${process.env.URLSERVERSTORAGE}/api/v2/server/upload`;
        ...
        res.json({
            success: true,
            videoUrl: `${process.env.URLSERVERSTORAGE}/api/v2/server/stream/${videoId}?tk=${token}&expires=${expires}`,
            videoID: videoId
        });
